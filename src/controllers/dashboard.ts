...
export const getDashboardStats = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const [
      ...
      Finding.countDocuments({ ...filter, severity: "critical", resolvedAt: null }),
      Finding.countDocuments({ ...filter, severity: "high", resolvedAt: null }),
      Finding.countDocuments({ ...filter, severity: "medium", resolvedAt: null }),
    ...
    const totalOpenThreats = criticalFindings + highFindings + mediumFindings + lowFindings;

    // Calculate score (0 - 100)
    let score = 100;
    if (totalScans > 0) {
      const penalty = (criticalFindings * 25) + (highFindings * 15) + (mediumFindings * 5) + (lowFindings * 1);
 ...
 */
export const getInstallations = async (req: Request, res: Response): Promise<void> => {
  try {
    const { owner } = req.query;
 ...
 */
export const getInstallationRepos = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!installation) {
      res.status(404).json({ error: `Installation not found for owner '${owner}'` });
      return;
    }
    ...
    const { data: reposData } = await client.request("GET /installation/repositories", {
      per_page: 100,
    });

    const reposWithStats = await Promise.all(
      reposData.repositories.map(async (r: { id: number; name: string; full_name: string; private: boolean; default_branch: string }) => {
        const [latestScan, openFindingsCount] = await Promise.all([
          Scan.findOne({ installationId: installation.installationId, repo: r.name })
            .sort({ startedAt: -1 })
            .lean(),
    ...
    const message = err instanceof Error ? err.message : String(err);
    logger.error(`[dashboard] Failed to get installation repos for ${req.params.owner}: ${message}`);
    res.status(500).json({ error: "Internal server error" });
  }
};
 ...
 */
export const scanSingleRepository = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!installation) {
      res.status(404).json({ error: `Active installation not found for owner '${owner}'` });
      return;
    }
    ...
    const message = err instanceof Error ? err.message : String(err);
    logger.error(`[dashboard] Failed to trigger scan for ${req.params.owner}/${req.params.repo}: ${message}`);
    res.status(500).json({ error: "Internal server error" });
  }
};
 ...
 */
export const getRepoFixPRs = async (req: Request, res: Response): Promise<void> => {
  try {
    const { owner, repo } = req.params;
    const installation = await Installation.findOne({
      owner: new RegExp(`^${owner}$`, "i"),
    ...
    const { data: pulls } = await client.request("GET /repos/{owner}/{repo}/pulls", {
      owner,
      repo,
      state: "open",
      per_page: 50,
    ...
    const message = err instanceof Error ? err.message : String(err);
    logger.error(`[dashboard] Failed to fetch PRs for ${req.params.owner}/${req.params.repo}: ${message}`);
    res.status(500).json({ error: "Internal server error" });
  }
};
 ...
 */
export const approveFixPR = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    logger.error(`[dashboard] Failed to approve PR #${req.params.pull_number}: ${message}`);
 ...
 */
export const mergeFixPR = async (req: Request, res: Response): Promise<void> => {
  try {
    const { owner, repo, pull_number } = req.params;
    const pullNumber = parseInt(pull_number, 10);
    ...
    const message = err instanceof Error ? err.message : String(err);
    logger.error(`[dashboard] Failed to merge PR #${req.params.pull_number}: ${message}`);
    res.status(500).json({ error: message });
  }
};
